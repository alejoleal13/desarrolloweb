...
export class DashboardComponent implements OnInit {
  ...
  Consulta(artista:Artista){
    this.service.consultasemestre(artista)
    .subscribe(data=>{
      this.artista=this.artista.filter(s=>s!==artista);
      this.router.navigate(["consultaartista"])
    })

  }
  ngOnInit() {
    
    this.service.getArtista().subscribe(data=>{this.artista=data;})
     ...
     const optionsCompletedTasksChart: any = {
      ...
      var datawebsiteViewsChart = {
        labels: ['Aprobado', 'Reprobado', 'Cancelado'],
        series: [
          [20, 40, 8]
